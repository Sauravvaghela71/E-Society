...
import { 
  FaSignOutAlt, FaTachometerAlt, FaRegUser, FaExclamationCircle, 
  FaCalendarAlt, FaTools, FaUserFriends
} from "react-icons/fa";
...
export default function UserSidebar() {
  ...
  return (
    <div className="w-64 h-screen bg-gray-900 text-white p-6 fixed flex flex-col justify-between shadow-2xl border-r border-gray-800 z-50">
      ...
      <div>
        <div className="flex items-center gap-3 mb-10 px-2 cursor-pointer" onClick={() => navigate("/")}>
          ...
          <div>
            ...
            <span className="text-[10px] text-gray-500 font-bold tracking-[0.2em] uppercase">Resident Portal</span>
          </div>
        </div>
        ...
        <nav className="custom-scrollbar overflow-y-auto max-h-[calc(100vh-250px)]">
          <ul className="space-y-1">
            <SidebarLink 
              ...
              label="Dashboard"
              active={isActive("/user/dashboard") || isActive("/user")}
            />
            <SidebarLink 
              to="/user/profile" 
              icon={<FaRegUser />} 
            ...
            <SidebarLink 
              ...
              label="My Complaints" 
              active={isActive("/user/complaint")} 
            />
            <SidebarLink 
              to="/user/facilities" 
              icon={<FaCalendarAlt />} 
              label="Book Amenities" 
              active={isActive("/user/facilities")} 
            />
            <SidebarLink 
              to="/user/maintenance" 
              icon={<FaTools />} 
              label="Maintenance Bills" 
              active={isActive("/user/maintenance")} 
            />
            <SidebarLink 
              to="/user/visitor" 
              icon={<FaUserFriends />} 
              label="My Visitors" 
              active={isActive("/user/visitor")} 
            />
          </ul>
        </nav>
      </div>
